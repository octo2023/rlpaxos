...
type Node interface {
	...
	RelpyForward(c Command, resp Reply)
}
...
func (n *node) recv() {
	for {
		...
		switch m := m.(type) {
		case Request:
			log.Debugf("node %v received request %v", n.id, m)
...
func (n *node) RelpyForward(c Command, resp Reply) {
	n.RLock()
	r, ok := n.forwards[c.String()]
	if !ok {
		return
	}
	n.RUnlock()
	r.Reply(resp)
	log.Debugf("node %v received reply %v", n.id, c)
}
